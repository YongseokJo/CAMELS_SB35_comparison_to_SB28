...
#save_prefix = "SB35_cutout_conv_no_linear_sig_no_monopole"
#save_prefix = "SB35_cutout_conv_no_linear_om_no_monopole_2"
#save_prefix = "SB35_cutout_conv_no_linear_sig_no_monopole_2"
#save_prefix = "SB35_cutout_conv_no_linear_sig"
#save_prefix = "SB35_cutout_conv_no_linear_sig_2"
save_prefix = "SB35_cutout2_conv_no_linear_sig" ## the order of cutous is in
## cutout octant jupyter notebook
...
if individual:
    data, labels,minmax = \
            loadCAMELS(field="Mtot",box="SB35",normalization=False,
                       linear=False, buffer=0.3, individual=False)
    ...
    #data = data[idx,:256,:256]
    data = data[idx,256:,256:]
    sums = data.sum(axis=(1, 2), keepdims=True)     # shape = (N,1,1)
    data = data/sums

    data  = np.log10(data)
    means = data.mean()     # shape = (N,1,1)
    stds  = data.std()     # shape = (N,1,1)
    data  = (data-means)/stds
    data  = torch.tensor(data,dtype=dtype).to(device)

    labels = (labels-minmax[:,0])/(minmax[:,1]-minmax[:,0])
    ...
    data = torch.tensor(data[idx,:256,:256],dtype=dtype).to(device)
