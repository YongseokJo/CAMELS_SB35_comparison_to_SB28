...
sim = "IllustrisTNG"
#sim = "SIMBA"
#save_prefix = f"TNG_SIMBA_T_LH_sig_no_linear"
save_prefix = f"TNG_SIMBA_T_LH_om_no_linear"

data1, labels1,minmax1 = loadCAMELS(sim="IllustrisTNG",
                                 field="T",box="LH",normalization=False,
                                 linear=False)
data2, labels2,minmax2 = loadCAMELS(sim="SIMBA",
                                 field="T",box="LH",normalization=False,
                                 linear=False)
...
labels1 = labels1*(minmax1[:,1]-minmax1[:,0])+minmax1[:,0]
labels2 = labels2*(minmax2[:,1]-minmax2[:,0])+minmax2[:,0]
labels = np.concatenate([labels1, labels2], axis=0)

buffer = 0.3
_min = labels.min(axis=0)
_max = labels.max(axis=0)
_min -= (_max-_min)*(1-buffer)
_max += (_max-_min)*buffer
labels = (labels-_min)/(_max-_min)

minmax =  np.c_[_min,_max]
